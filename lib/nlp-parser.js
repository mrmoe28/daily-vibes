...
class NLPParser {
    ...
    extractTime(input) {
        ...
        // Check for simple time patterns like "1pm", "2am", "11:30am"
        if (!result.startTime) {
            const simpleTimeMatch = input.match(/\b(\d{1,2})(?::(\d{2}))?\s*(pm|am|PM|AM)\b/);
            if (simpleTimeMatch) {
                let hours = parseInt(simpleTimeMatch[1]);
                const minutes = simpleTimeMatch[2] || '00';
                const period = simpleTimeMatch[3].toLowerCase();
                
                if (period === 'pm' && hours !== 12) hours += 12;
                if (period === 'am' && hours === 12) hours = 0;
                
                result.startTime = `${hours.toString().padStart(2, '0')}:${minutes}`;
            }
        }
